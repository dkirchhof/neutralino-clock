...
import { useReact_useEffect_Z101E1A95, React_createDisposable_3A5B6456, useReact_useEffectOnce_Z5ECA432F, useReact_useRef_1505 } from "../fable_modules/Feliz.1.56.0/React.fs.js";
...
export function TimerState$reflection() {
    return union_type("Timer.TimerState", [], TimerState, () => [[["Item", float64_type]], []]);
}
...
export function Props$reflection() {
    return record_type("Timer.Props", [], Props, () => [["startTime", int32_type], ["onCancel", lambda_type(unit_type, unit_type)]]);
}

export function Timer(props) {
    const audioPlayer = useReact_useRef_1505(void 0);
    const patternInput = useRefState(props.startTime);
    const setRemainingTime = patternInput[1];
    const remainingTimeRef = patternInput[2];
    const remainingTime = patternInput[0] | 0;
    ...
    const addOrSubtractTime = (delta) => {
        setRemainingTime(max((x, y) => comparePrimitives(x, y), 0, remainingTimeRef.current + delta));
    };
    const startTimer = () => {
        if (remainingTimeRef.current > 0) {
            ...
            const id = window.setInterval(subtractOneSecond, 1000);
    ...
    useReact_useEffect_Z101E1A95(() => {
        const matchValue_2 = [timerStateRef.current, remainingTime];
        let pattern_matching_result, id_4;
        if (matchValue_2[0].tag === 0) {
            if (matchValue_2[1] === 0) {
                pattern_matching_result = 0;
                id_4 = matchValue_2[0].fields[0];
            }
            else {
        ...
        switch (pattern_matching_result) {
            case 0: {
                value_26(audioPlayer.current).play();
                stopTimer(id_4);
                break;
            }
            case 1: {
    ...
    const patternInput_2 = splitTime(remainingTime);
    const seconds = patternInput_2[2] | 0;
    const minutes = patternInput_2[1] | 0;
    const hours = patternInput_2[0] | 0;
    return react.createElement(react.Fragment, {}, createElement("audio", {
        ref: audioPlayer,
        src: "./audio/mixkit-bell-notification-933.wav",
    }), createElement("div", {
